...
description: >-
  ...
  Don't use for analyzing source code or extracting raw insights from JSONL files.
---

# Perimeter (/ray-perimeter)

## System Goal

Threat Boundary Architect. Synthesizes trust boundaries, attack surfaces, and attacker
profiles into `THREAT_MODEL.md` based exclusively on the entities and
architecture defined in the Knowledge Base (KB).

## Command Definition

- **Command:** `/ray-perimeter`
...
- **Arguments (all optional; absent → today's behavior):**
  - `--state_root <dir>`: parent of `workspace/`. If absent, use
    ...
    `.ray_state.json`, the KB, and the archive directory.
...
- **Reads**:
  - `workspace/.ray_state.json` — `pass_number` (to archive per pass) and,
...
**Role for this stage (findings-only / KB-only).** `/ray-perimeter` NEVER
reads target source. Every input it uses — `architecture.md`, `entities/*.md`,
the prior `THREAT_MODEL.md`, and `.ray_state.json` — is STATE-RELATIVE (read
...
0. **Resolve Snapshot Provenance, Freshness, and Archive the Prior Model:**
   ...
   - Resolve `N`: read `"pass_number"` from `workspace/.ray_state.json`. If
...
1. **Read the Synthesized KB:**
   ...
   - Read the files inside `workspace/kb/entities/` to understand the individual
     ...
     to them by the `/ray-blueprint` stage.
...
3. **Synthesize the Threat Model:**
   ...
   - **Deployment Intent:** State exactly one of `Intent: PRODUCTION` or
     ...
     `/ray-magistrate` marks EVERY finding `SAMPLE_OR_TEST` (dismissing the whole
...
- The FIRST line of `workspace/kb/THREAT_MODEL.md` MUST be the provenance stamp
  ...
  `PREV_TM`, and `ray-magistrate` reads it as a KB freshness check.
