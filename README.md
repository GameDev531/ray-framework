...
<p><strong>Start here, whatever your tool.</strong> <code>AGENTS.md</code> is the tool-agnostic <strong>routing map</strong> — it tells an agent which one or two skills a task needs so it never reads all 33. Tool-native entry files load it automatically and point back to it: <code>CLAUDE.md</code> (Claude Code), <code>GEMINI.md</code> (Gemini CLI), <code>.cursor/rules/ray-framework.mdc</code> (Cursor), <code>.github/copilot-instructions.md</code> (Copilot), and <code>AGENTS.md</code> itself (Codex and other AGENTS-aware runtimes). <code>docs/coverage-map.md</code> records what the red/blue agents cover and the deliberate charter boundary (red performs the local slice; blue detects the whole kill chain). <code>docs/usage-guide.md</code> is the operating manual — when to use each skill, when to use each agent, and the exact order to run a complete audit (the static skill pipeline plus the agents' live tool flow).</p>
...
<ul>
  <li><strong>Packaging:</strong> Installs as a Claude Code plugin (<code>ray@ray-framework</code>) via the bundled <code>.claude-plugin/</code> marketplace; validated with <code>claude plugin validate --strict</code>, all 33 skills and 5 subagents load, plus the <code>ray-tools</code> MCP server.</li>
  ...
  <li><strong>Real-Browser QA:</strong> <code>ray-vantage</code> plus the <code>ray-usher</code> subagent &mdash; drives the pre-installed Chromium against your running app to prove front-to-back coherence (a backend feature with no button, a button wired to nothing), complete the core user flows, and re-verify that siege/domain fixes hold from the user's seat &mdash; returning a 1&ndash;5 verdict with screenshot/network evidence and the frontend wiring fixes for any gaps.</li>
  <li><strong>Legal &amp; Compliance:</strong> <code>ray-counsel</code> &mdash; a legal-engineering layer that audits AI transparency (FTC-style deceptive-practice risk), advertising claims, Terms of Service &amp; arbitration/class-action posture, the privacy data-map, third-party SDK data flows, and the Apple App Privacy / Google Play Data Safety labels &mdash; always jurisdiction-first, reconciling the CODE&nbsp;&rarr;&nbsp;DOCS&nbsp;&rarr;&nbsp;UI&nbsp;&rarr;&nbsp;store-listing so they describe the same reality. It classifies each item (mandatory-by-law / platform-requirement / best-practice / risk / needs-counsel), never invents a law, and never certifies a project as legally safe &mdash; it is not a substitute for a qualified lawyer.</li>
