...
name: ray-compass
description: >-
  Formulates a targeted defensive security reviewing plan based on the active threat model and historical insights.
...
Review Strategist. Analyzes code structure, directory metadata, and historical
...
- **Description:** Formulates a targeted defensive security reviewing plan based
  on the active threat model and historical insights.
- **Arguments (optional; supplied by the orchestrator, consumed by Block A):**
  ...
  - `--state_root`: absolute path to the `workspace/` state dir (plan.json,
    .ray_state.json, findings/, kb/, archive/). STATE-RELATIVE — never
...
- **Reads**:
  ...
  - `workspace/.ray_state.json` NEW fields:
    `active_snapshot.{snapshot_id, snapshot_pinned, vcs_type}`,
    `snapshot_history` (read, written by the orchestrator, ray-conductor). `vcs_type` is read
...
> harness per `ray-foundry` Scenario 2):** if `active_snapshot` is
...
> re-pinning. The reference orchestrator (ray-conductor) re-pins every pass, so this check never
...
- `workspace/kb/`, `workspace/plan.json`, `workspace/.ray_state.json`,
...
2. **Determine Mode & Retrieve Insights:** Check if the knowledge base index
   ...
   - **MODE A: First-Pass Exhaustive Mode (No `workspace/kb/index.md` found):**
     ...
     1. Uses `find` or `os.walk` to crawl all production directories. If a
        `ray-prism.md` file exists in a directory, use its contents to
   ...
   - **MODE B: Strategic Learning Mode (`workspace/kb/index.md` exists):** Read
     ...
     - **Targeted Re-Evaluation & Retries**: Review the KB index, entity files,
       ...
       Also read the snapshot context from `workspace/.ray_state.json`:
       `active_snapshot.{snapshot_id, snapshot_pinned}` and `snapshot_history`
       (both written by the orchestrator, ray-conductor). Then COMPUTE `changed_files` /
       ...
       `workspace/.ray_state.json`, then use them for the rest of the stage.
       ...
       exactly the way `ray-detonator` writes it: if the finding has a
       ...
       2. **Copy for Retry (snapshot-gated) or Re-discover:** For each archived
          ...
          - **MODE-OFF bypass (3-state rule):** if `active_snapshot` is ABSENT
            ...
            This mirrors `ray-anvil`'s LEGACY-mode carve- out
            (mirroring `ray-anvil`'s own LEGACY-mode carve-out). Do NOT gate the
            bypass on `snapshot_pinned==false` — that would also catch HALT
            mode, where the STALE banner legitimately marks the finding as
            needing re-verification. Gate ONLY on `active_snapshot` absent. (Do
            NOT change Block B itself — it is character-identical across every
            Ray skill per the shared block-fidelity convention; the fix goes in
            Compass's CONSUMERS of Block B, not Block B.)
     ...
     - **Dependency-aware fan-out (Phase 2 incremental efficiency):** When
       ...
       - **When to use:** ONLY when `changed_files_status` is known AND the KB
         ...
         `workspace/.ray_state.json` against `SNAPSHOT_ID` — if they differ,
