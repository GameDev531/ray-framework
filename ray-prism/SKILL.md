...
name: ray-prism
description: >-
  Pre-processes the repository by generating security-focused summaries (ray-prism.md) for each directory to make planning and research more efficient.
...
Repository Digest Mapper. Automates the generation of security-focused, deterministic
...
- **Description:** Pre-processes the repository by generating security-focused
  summaries (`ray-prism.md`) for each directory to make planning and
...
- **Reads**:
  ...
  - Child directory summaries (`ray-prism.md` files from subdirectories).
  - `workspace/historical_insights.jsonl` (optional, to enrich summaries).
- **Writes**:
  - Traversal script to workspace.
  - MODE-OFF: `ray-prism.md` in each source directory (as today). PINNED:
    skipped (see Output location).
- **Preconditions**:
  - Source files and directory structure must be present.
- **Idempotency Guarantee**:
  - Deterministically overwrites existing `ray-prism.md` files in-place
...
- PINNED mode (snapshot_pinned true): summaries are **skipped** this pass. In
  ...
  history, researcher) read `ray-prism.md` from the source directory in the
  ...
  `ray-prism.md` files in PINNED mode. (If a future change wires consumers
...
- HALT mode (active_snapshot present + snapshot_pinned=false): behave as
  MODE-OFF (write `ray-prism.md` into each source directory). The snapshot
...
- MODE-OFF (no `active_snapshot` — today's default): behave exactly as today —
  write `ray-prism.md` into each source directory.
- In all modes except PINNED, `ray-prism.md` files must remain invisible to
  every VCS dirty check and be deleted from the target tree before any sync (the
  orchestrator (ray-conductor) enforces this in Block C STEP 0). Never let a summary make the tree
  look dirty.

Your task is to write and execute a script that will traverse the repository
directory tree and create a `ray-prism.md` file in each directory
containing source code.

This is an **optional pre-processing phase** designed to drastically reduce the
context window size required for the strategist (`/ray-compass`), and provide a
quick reference map for researchers (`/ray-prospector`).
...
1. **Write the Traversal Script (Bottom-Up Hierarchical):** Write a script
   ...
   - When analyzing a directory, the script should pass the LLM the local source
     files in that directory **PLUS** the `ray-prism.md` files of its
...
2. **Generate the Security Summary (Map-Reduce):** The script should read
   `workspace/historical_insights.jsonl` (if it exists) to check for past
   ...
   - **Historical Vulnerabilities & Fixes:** What files or components in this
     ...
     recorded in `workspace/historical_insights.jsonl`? Summarize the past
...
3. **Output to `ray-prism.md`:** In MODE-OFF (or HALT), write
   `ray-prism.md` into the corresponding source directory (overwrite if
