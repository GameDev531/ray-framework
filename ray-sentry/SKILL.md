...
name: ray-sentry
description: >-
  Hunts abuse and observability gaps: missing rate limiting, resource-exhaustion abuse, exposed internal/debug endpoints, unsigned webhooks, shadow/undocumented APIs, and verbose error leakage.
  Use when the target exposes public endpoints, webhooks, or expensive operations.
  Don't use for auth logic (ray-turnstile) or dependency CVEs (ray-manifest).
---
...
Abuse & Exposure Auditor. Finds the endpoints an attacker can hammer, reach that they shouldn't, or spoof — the operations with no rate limit, the internal routes left exposed, the webhooks with no signature, and the APIs no one documented.

## Command Definition

- **Command:** `/ray-sentry [--target_root=<path>] [--snapshot_root=<path>] [--snapshot_id=<id>] [--state_root=<path>]`
- **Description:** Reviews rate limiting, exposed/debug endpoints, webhook signatures, and shadow APIs.
...
- **Reads**:
  ...
  - `references/sentry-docket.md` — this skill's vulnerable→safe pattern catalogue
...
Read `references/sentry-docket.md`. From `workspace/plan.json`, take the
...
### Step 2: Hunt — what can be hammered, reached, or spoofed

Enumerate every route (including ones not in the docs — check route registration,
middleware, and config), and for each expensive or sensitive operation ask: is there a
rate/concurrency limit, and at which layer? Is this route meant to be internal (admin,
debug, metrics, health, actuator) and is it actually protected? For inbound webhooks,
is the signature verified? For errors, does the response leak stack traces, versions,
or internal paths?
