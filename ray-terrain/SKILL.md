...
name: ray-terrain
description: >-
  Hunts infrastructure-as-code, cloud, and container misconfiguration: Terraform/CloudFormation/Kubernetes/Docker settings, over-permissive IAM, public storage, and unhardened images.
  Use when the target ships IaC, Kubernetes manifests, Dockerfiles, or cloud config.
  Don't use for application code flaws (the app domain skills) or dependency CVEs (ray-manifest).
---
...
Infrastructure Auditor. Reads the declarative infrastructure — Terraform, CloudFormation, Kubernetes, Docker, cloud config — and finds the settings that expose data, over-grant identity, or ship an unhardened runtime, before any of it reaches an account.

## Command Definition

- **Command:** `/ray-terrain [--target_root=<path>] [--snapshot_root=<path>] [--snapshot_id=<id>] [--state_root=<path>]`
- **Description:** Reviews IaC, cloud, and container configuration for misconfiguration and over-permissioning.
...
- **Reads**:
  ...
  - `references/terrain-docket.md` — this skill's vulnerable→safe pattern catalogue
...
Read `references/terrain-docket.md`. From `workspace/plan.json`, take the
...
### Step 2: Hunt — the config that provisions the blast radius

Read the IaC and container definitions. For each resource ask: is it exposed to the
public that shouldn't be (open security groups/0.0.0.0, public buckets/blobs), does an
identity grant more than it needs (`*` actions/resources, wildcard trust, privileged
service accounts), and is the runtime hardened (non-root, read-only FS, no
`privileged`/`hostNetwork`, dropped capabilities, pinned base image)? Check that secrets
are not baked into IaC or image layers.
...
the spec or by execution; (6) it is not a designed behavior for a trusted actor.
 A misconfiguration is VALID from the declarative source even without a running deployment to test — but describe the concrete exposure it creates.
