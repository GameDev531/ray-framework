...
# Magistrate (/ray-magistrate)

## System Goal

Production Viability Judge. Filters validated security findings to confirm if
they remain triggerable in standard release and production configurations.

## Command Definition

- **Command:**
  `/ray-magistrate [--target_root=<path>] [--snapshot_root=<path>] [--snapshot_id=<id>] [--state_root=<path>]`
...
- **Reads**:
  ...
  - `workspace/.ray_state.json` (to track current loop pass and to read
...
- **Writes**:
  ...
  - Appends to `workspace/insights.jsonl`.
- **Preconditions**:
...
- **Idempotency Guarantee**:
  - Overwrites viability fields in place. It must check if a critic entry for
    ...
    `workspace/insights.jsonl` to ensure it does not write duplicate records if
...
Magistrate is a CODE-READING stage (it inspects the target source), so it is NOT a
...
`workspace/.ray_state.json`. If neither is present (no `--snapshot_id` AND no
...
2. **Evaluate Global Repository Intent (KB-freshness gated):** Read
   ...
   **KB freshness gate — REQUIRED before any blanket mass-mark (3-state rule):**
   ...
   2. Else the `kb_snapshot_id` value in `workspace/.ray_state.json` (which
...
3. **Acquire Targeted Code Snippets (snapshot-matched):** For each finding where
   ...
   **Otherwise (drift, missing file, or out-of-range):** set
   ...
   telemetry recorded by `ray-detonator` (e.g. `build_profile`,
...
7. **Append to Long-Term Memory:** For each finding you loaded (including
   ...
   database file named `workspace/insights.jsonl` (using append mode). This
