...
name: ray-seam
description: >-
  Hunts client/server trust-boundary flaws: input validation gaps, mass-assignment, CORS misconfiguration, cache poisoning, host-header injection, clickjacking, weak/missing CSP, and open redirects.
  Use when the target exposes a browser-facing web surface or an HTTP API boundary.
  Don't use for deep auth logic (ray-turnstile) or injection sinks (ray-crucible).
---
...
Boundary Inspector. Audits the seam where the client meets the server — the headers, origins, redirects, and body-binding rules that decide whose requests are trusted and what a browser is allowed to do with the response.

## Command Definition

- **Command:** `/ray-seam [--target_root=<path>] [--snapshot_root=<path>] [--snapshot_id=<id>] [--state_root=<path>]`
- **Description:** Reviews the client/server boundary: CORS, CSP, host header, cache, redirects, mass assignment.
...
- **Reads**:
  ...
  - `references/seam-docket.md` — this skill's vulnerable→safe pattern catalogue
...
Read `references/seam-docket.md`. From `workspace/plan.json`, take the
...
### Step 2: Hunt — the headers and origins that gate the browser

Read the CORS configuration, the CSP, the cookie and framing headers, the redirect
handlers, the cache keys, and the request-body binding. For each, ask what a
cross-origin page or a header-spoofing client can make the server do or the browser
accept. Trace where the app trusts the `Host`/`X-Forwarded-*` headers, where a
response is cached under a key that omits a security-relevant input, and where a
user-supplied URL feeds a redirect.
