...
name: ray-turnstile
description: >-
  Hunts identity & access flaws: broken authentication, session/JWT/OAuth handling, MFA, password reset/invite, and authorization — IDOR/BOLA, BFLA, tenant isolation, the OWASP API Top 10.
  Use when the target has login, sessions, roles, tenants, or an authenticated API.
  Don't use for input-injection (ray-crucible) or transport/cookie hygiene alone (ray-custodian).
---
...
Access-Control Judge. Verifies not just that authentication and authorization checks exist, but that each checks the RIGHT permission on the RIGHT resource via the RIGHT mechanism — and that no parallel path reaches the same state change with a weaker (or absent) check.

## Command Definition

- **Command:** `/ray-turnstile [--target_root=<path>] [--snapshot_root=<path>] [--snapshot_id=<id>] [--state_root=<path>]`
- **Description:** Reviews authentication and authorization for bypasses, IDOR/BOLA, BFLA, and broken tenancy.
...
- **Reads**:
  ...
  - `references/turnstile-docket.md` — this skill's vulnerable→safe pattern catalogue
...
Read `references/turnstile-docket.md`. From `workspace/plan.json`, take the
...
### Step 2: Hunt — who can do what, to whose data

Map every state-changing and data-reading operation to its permission check.
For each: is it the right check, on the right object, and can a request-body field
override what the check intended to restrict? Enumerate object identifiers (IIDs in
URLs/bodies) and test horizontal access (another user's/tenant's id) and vertical
access (an action gated to a higher role). Check bulk/batch/export/import paths for
per-item enforcement. For auth itself, trace credential storage, session creation and
rotation, token validation, and every account-recovery flow (reset, invite, MFA) for a
bypass.
...
the spec or by execution; (6) it is not a designed behavior for a trusted actor.
 An authorization gap is VALID even where the UI never exposes the path — the API does.
...
`severity`, `cwe`, `owasp` (map to A01:2021 / the API Top 10, e.g. `API1:2023` BOLA, `API5:2023` BFLA), `code_paths` (SNAPSHOT-RELATIVE `path:line`),
