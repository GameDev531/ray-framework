...
- **Arguments (optional; supplied by the orchestrator, consumed by Block A):**
  ...
  - `--state_root`: absolute path to the `workspace/` state directory
    (`plan.json`, `.ray_state.json`, `findings/`, `kb/`). State paths are
...
- **Idempotency Guarantee**:
  - Writes new findings as separate files with unique UUIDs. Rely on
    `ray-condenser` to cluster and merge duplicate findings on subsequent steps.
...
- `kb_references`, `workspace/plan.json`, `workspace/.ray_state.json`, and
...
1. **Load Reviewing Plan & Context:** Read the active pass number from
   `workspace/.ray_state.json` and resolve the current ISO 8601 timestamp.
   ...
   from `workspace/.ray_state.json` (`root`, `snapshot_id`,
