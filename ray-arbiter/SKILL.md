...
Independent Finding Validator. Reviews consolidated findings against active source code
...
- **Reads**:
  ...
  - `workspace/.ray_state.json` (current loop `pass_number`; and, when
...
- **Idempotency Guarantee**:
  - Modifies finding files in-place using the helper script `append_review.py`,
    which MUST carry `# RAY_HELPER_VERSION = 2` as its first line; regenerate
...
`workspace/.ray_state.json` has no readable `active_snapshot`. This is a
...
- **NOT_MATCHED** (includes: `snapshot_pinned` false / HALT / live-endpoint
  ...
  - `"triage_checklist"`: all 13 constraints set to `UNKNOWN` (paste the object
    ...
    (per schema.json's `triage_checklist` requirement), and for a
    `NEEDS_RESEARCH` finding every entry MUST avoid `FAIL`/`passes:false` (per
    schema.json's status/checklist consistency rule).
...
3. **Strict Validation Filtering (Apply the 13 Negative Constraints):** Evaluate
   ...
   **Profile Review Overrides (read FIRST, before applying the 13 rules).** Read
   ...
   - `rate_limit` / `resource_exhaustion` → **rule 07** MUST NOT auto-dismiss a
     missing rate/lockout limit on auth/OTP/expensive endpoints (`web-app` /
     `llm-app` / native-parser profiles set this).
   ...
   - **Status Resolution:**
     ...
     - **SCHEMA-CRITICAL:** `FALSE_POSITIVE` is the ONLY status for which a
       ...
       `FAIL` — or schema.json's status/checklist consistency rule will reject
       the finding. If a
   ...
   - **Checklist Construction:**
     ...
     - Construct the `triage_checklist` object evaluating all 13 negative
       ...
       - `"FAIL"`: if the finding violates the rule. Setting ANY entry to
         ...
         (schema.json's status/checklist consistency rule rejects `FAIL` on
         `VALID`/`PROVISIONALLY_VALID`/`NEEDS_RESEARCH`). A `FAIL` entry also
         REQUIRES a `reason` (per schema.json's `triage_checklist` entry
         schema).
     ...
     - Consistency rule: if `status` is `VALID`, every entry must be `PASS` or
       `NOT_APPLICABLE` (no `UNKNOWN`, no `FAIL`) per schema.json's
       `VALID`-status consistency rule.
...
5. **Token-Optimized File Updates:** To minimize LLM output tokens, **do not
   ...
   **Helper version guard (mechanical):** the helper's FIRST line MUST be the
   comment `# RAY_HELPER_VERSION = 2`. Before reusing an existing
