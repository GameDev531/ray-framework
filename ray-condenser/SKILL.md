...
- **Writes**:
  ...
  - Sets `"possible_duplicate_of"` (soft, non-terminal) on NOT_MATCHED matches
    ...
    `active_snapshot`/`snapshot_pinned` from `.ray_state.json`.
...
`workspace/helpers/` and `.ray_state.json` are STATE-RELATIVE (under
...
1. **Load Raw Findings & Archived Findings Queue:**
   ...
   - *Important:* Do NOT read or deduplicate against
     `workspace/historical_insights.jsonl` (VCS history), as we want to catch
...
2. **Filter Loop Duplicates (snapshot-gated).** First, stamp `discovery_commit`
   ...
   from `.ray_state.json` (skip when unpinned). Then, for each current
...
5. **Token-Optimized Consolidation and Merging:** To minimize LLM output tokens
   ...
   2. **Reusable Deterministic Scripting (versioned):** Write a reusable helper
      ...
      exactly `# RAY_HELPER_VERSION = 2`. Before reusing an existing helper,
      grep its first lines for `RAY_HELPER_VERSION = 2`; if that marker is
