...
name: ray-blueprint
description: >-
  Synthesizes raw insights and codebase analysis into an interlinked Markdown Knowledge Base (KB).
...
# Blueprint (/ray-blueprint)

## System Goal

Knowledge Base Synthesizer. Translates ephemeral insights from the insights
queue (`workspace/insights.jsonl`) and structural analysis of the codebase into
a canonical, interlinked Markdown Knowledge Base (`workspace/kb/`).

## Command Definition

- **Command:** `/ray-blueprint`
...
- **Reads**:
  - `workspace/insights.jsonl` (raw insights from the current round).
  - `workspace/historical_insights.jsonl` (optional, past vulnerability
    metadata).
  - Codebase directory structure and key source files.
  - Existing Markdown files in `workspace/kb/` (to validate/decay check).
  - `workspace/.ray_state.json` (to retrieve pass count).
  - `workspace/.ray_state.json` → `active_snapshot` (`root`, `snapshot_id`,
  ...
  - `workspace/.ray_state.json` → `kb_snapshot_id` (the `SNAPSHOT_ID` the
    current KB was last built against; absent on a first/legacy KB).
  - `workspace/.ray_state.json` → `changed_files` and `changed_files_status`
    (written by ray-compass's Block E; consumed by the scoped KB invalidation in
...
- **Writes**:
  ...
  - `workspace/kb/dependencies.json` — a JSON map of import/dependency edges
    ...
    This is consumed by `ray-compass`'s dependency-aware fan-out (Phase 2). If
  ...
  - Archives `workspace/insights.jsonl` to
    `workspace/archive/insights/insights_pass_${N}_${X}.jsonl`.
...
- **Idempotency Guarantee**:
  ...
  - Snapshot stamping is part of the same transaction: the `KB_SNAPSHOT`
    ...
    insights move. On any failure, leave `workspace/insights.jsonl` intact.

## Instructions

Analyze the codebase and pending insights to construct a permanent,
...
- `KB_ID` = the `kb_snapshot_id` value in `.ray_state.json` (primary); else the
     ...
     - **Scoped invalidation (Phase 2 incremental efficiency):** If
       ...
       on every (re)written file.
        - **Parent-rollup (2-hop, matching plan's fan-out):** When
          ...
          `ray-compass`'s dependency-aware fan-out (which expands up to 2
...
1. **Read the Inbox (`workspace/insights.jsonl` and
   `workspace/historical_insights.jsonl`):**

   - Parse the contents of `workspace/insights.jsonl` (and
     `workspace/historical_insights.jsonl` if it exists). Extract all
...
3. **Build or Update the Knowledge Base (KB):**
   ...
   - **Per-file `KB_SNAPSHOT` stamping is the sole provenance mechanism for KB
     ...
     `ray-magistrate` Block B). A later-reverted fix is caught by these
...
5. **Transactional Inbox Clearing & Archiving:**
   ...
   - **Commit by Moving:** Only after verifying synthesis success, move
     `workspace/insights.jsonl` to the archive directory:
     - Ensure the target directory exists (e.g.,
       `mkdir -p workspace/archive/insights/`).
     - Determine the loop pass number `N` by reading `"pass_number"` from
       `workspace/.ray_state.json`. If missing or invalid, scan
     ...
     - Determine the sub-index `X` by counting existing files matching
       `insights_pass_${N}_*.jsonl` in `workspace/archive/insights/` and
       adding 1.
     - Move the file:
       `mv workspace/insights.jsonl workspace/archive/insights/insights_pass_${N}_${X}.jsonl`.
   ...
   - **Stamp state:** Write `kb_snapshot_id` = `SNAPSHOT_ID` (`CUR` from step
     0b) into `workspace/.ray_state.json`. If `active_snapshot` was absent
   ...
   - If synthesis fails or is interrupted, leave `workspace/insights.jsonl`
     intact in its original location to ensure no data is lost.
   - If `workspace/insights.jsonl` is ABSENT on entry (e.g. the Stage-15
     invocation, because the Stage-2 invocation already archived it this pass),
     skip ONLY the insights move; STILL run the freshness gate (step 0b), stamp
