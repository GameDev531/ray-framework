...
Risk Calibration Analyst. Evaluates confirmed findings against a rigorous risk
...
- **Writes**:
  - Updates finding files in-place with scoring/calibration fields
    ...
    `inferred_exposure`, `attacker_position`, `ray_risk_score`, `priority`,
...
`state_root/workspace/.ray_state.json` (Block A step 0/1c). NEVER stop if it
...
3. **Critical Sanity Triage (Downgrading & Capping Findings):** Before
   ...
   **Core Principle - Marginal Capability:** The final severity and priority of
   ...
   [Severity Docket](references/severity_docket.md). You MUST evaluate each
...
5. **Token-Optimized File Updates:** To minimize LLM output tokens, **do not
   ...
   - **Helper version marker (mandatory).** The very first line of
     ...
     `# RAY_HELPER_VERSION = 2`. Before reusing an existing helper, read its
     first line: if the file is absent, OR the first line is not exactly
     `# RAY_HELPER_VERSION = 2` (marker missing or a different integer),
   ...
   - `"ray_risk_score"` (the final Hazard score)
