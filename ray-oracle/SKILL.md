...
name: ray-oracle
description: >-
  Hunts flaws in the target's OWN AI/LLM feature: prompt injection (direct and indirect), MCP tool poisoning, insecure output handling, excessive agency, model/data extraction — the OWASP LLM Top 10 (2025) and MITRE ATLAS.
  Use when the target builds prompts from untrusted input, calls tools from model output, or exposes an agent/MCP server.
  Don't use for a codebase with no AI feature.
---
...
AI-Feature Auditor. Audits the application's own LLM/agent surface — where untrusted text becomes part of a prompt, where model output drives an action or a tool call, and where an agent's authority exceeds what a compromised prompt should be allowed to do.

## Command Definition

- **Command:** `/ray-oracle [--target_root=<path>] [--snapshot_root=<path>] [--snapshot_id=<id>] [--state_root=<path>]`
- **Description:** Reviews the app's own LLM/agent feature for prompt injection, tool poisoning, and excessive agency.
...
- **Reads**:
  ...
  - `references/oracle-docket.md` — this skill's vulnerable→safe pattern catalogue
...
Read `references/oracle-docket.md`. From `workspace/plan.json`, take the
...
### Step 2: Hunt — untrusted text → prompt → action

Map the app's AI data flow: what untrusted content enters a prompt (user messages,
retrieved documents, tool results, web pages), what the model's output then drives (a
tool call, a shell/db action, a rendered response, a downstream prompt), and what
authority the agent holds. Ask: can injected instructions in the content override the
system prompt? Is model output treated as trusted (rendered as HTML, executed, used in a
query)? Can a poisoned tool/MCP server or document steer the agent? Does the agent have
more capability than any single request should wield?
...
`severity`, `cwe`, `owasp` (map to the OWASP LLM Top 10, e.g. `LLM01:2025` prompt injection, `LLM02:2025` insecure output handling), `code_paths` (SNAPSHOT-RELATIVE `path:line`),
