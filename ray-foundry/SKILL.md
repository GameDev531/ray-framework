...
description: >-
  ...
  Use when a user wants to build their own pipeline to wrap and run Ray skills reliably.
...
implementing their own deterministic orchestrator harness for Ray Skills.
...
- **Reads**:
  ...
  - `workspace/.ray_state.json` fields `active_snapshot`, `snapshot_history`,
  ...
  - `workspace/insights.jsonl` (to understand memory rotation).
...
5. **State Store & Memory Rotation:** To prevent token bloat and infinite loops,
   ephemeral queues (like `workspace/insights.jsonl`) must be rotated. Upon
   ...
   `mkdir -p workspace/archive/insights/`) and **move**
   `workspace/insights.jsonl` to a numbered archive (e.g.,
   `workspace/archive/insights/insights_pass_${N}_${X}.jsonl` where `${N}` is
...
- **LLM Output:** Generates a unique UUID for each finding and writes
  ...
  standard schema in [Prospector](../ray-prospector/SKILL.md)).
...
3. **Harness Action (Deterministic):**
   ...
   - Programmatically merges fields following the rules in
     [Condenser](../ray-condenser/SKILL.md) (e.g., union of
...
#### C. Validation & Review Stages (Arbiter, Magistrate)
...
### 2. Adaptable Detonators via Custom MCP
...
- **Architecture**:
  `[Detonator Agent] <--- MCP ---> [Custom MCP Server] <--- API ---> [Target Env]`
- **Custom Environments**:
...
(e.g., multiple workers running the `Detonator` stage in parallel):
...
- **Parallel Trajectory Search**: For deep reasoning stages (`Detonator`,
  `Anvil`), spawn multiple parallel agents attempting to solve the exact same
  finding using diverse logic paths. For the `Detonator` stage, prune all other
  trajectories as soon as one worker succeeds to save compute costs while
  escaping LLM "give up" loops. For the `Anvil` stage, wait for all patches to
...
- *Frontier Models*: For deep reasoning (Prospect, Detonate, Patch).
- *Flash/Lite Models*: For structured utility tasks (Condense, Calibrate).
- *Variability*: Run different models in parallel during the Prospecting stage
  to increase bug-hunting coverage.
...
Compass (`/ray-compass`) generates `workspace/plan.json` to define
targeted investigations, context pointers, and specific questions for the
auditor. Prospector (`/ray-prospector`) reads `workspace/plan.json` at
...
that `ray-conductor` implements. To support *continue-after-edits* and
...
Ray stage already carries in its own `SKILL.md`.

Ray runs under multiple harnesses (various CLIs, ADK, custom deterministic
pipelines), so the lifecycle must not live only in `ray-conductor`. Any
...
- **Store snapshots OUTSIDE `workspace/`.** The pinned copy (`SNAPSHOT_ROOT`)
  must live under `<state_root>/.ray_snapshots/pass_<N>` (or a clean-VCS
  worktree/archive), and its path **must not** contain the segment `/workspace/`
  — otherwise `ray-anvil`'s state-vs-code path guard misfires. Keep the last
...
`<state_root>/.ray_snapshots/pass_<N>` (or a clean-VCS worktree/archive), and
its path **must not** contain the segment `/workspace/` — otherwise
`ray-anvil`'s state-vs-code path guard misfires (state files appear to be
...
The reference harness (`ray-conductor`) satisfies this by re-pinning every
...
sentinel-exempt path 1a in Block A) — `ray-anvil` passes
...
`ray-condenser` Block B pairwise check compares the CURRENT finding's
...
evidence; `not_attempted` vs `failed_to_reproduce`) — `ray-detonator` Block
...
`RAY_REACHED_ENTRYPOINT` to a sidecar file at the point just before the sink
...
After Stage 2 (`/ray-blueprint`) completes, chunks are extracted into
...
with the skill (same pattern as `ray-condenser`'s `merge_findings.py`). This
...
[references/ray-beacon.md](references/ray-beacon.md). Builders can
...
(`RAY_HELPER_VERSION = 1`), and the JSON output schema.
...
orchestrator (or ray-conductor) when invoking the skill — the skill files themselves are not
modified:

- **ray-blueprint:** No changes needed. The harness chunks the existing
...
- **ray-compass:** If a retrieval mechanism is available, instruct the planner
...
- **ray-prospector:** If a retrieval mechanism is available, instruct Wave 1
...
(`/ray-condenser`) must compare each current finding against every archived
...
space before invoking `/ray-condenser`. The skill's existing deterministic
matching (`code_paths` + `title` + `discovery_commit`) remains the **sole
authority** for hard dedup decisions.

**This is entirely harness-side and opt-in.** `/ray-condenser` is not modified.
...
4. **ray-condenser invocation:** The harness invokes `/ray-condenser` as usual.
...
- **Fallback on failure.** If the embedding computation is unavailable, the
  candidate manifest is absent, or any error occurs, `/ray-condenser` falls back
...
Ray's discovery engine is 100% LLM-generative (grep swarm + reasoning). A
...
- **Option A (Default — Skill-Based):** A dedicated skill (`/ray-graft`)
  reads the IR and uses LLM reasoning to normalize findings into Ray finding
...
[references/ray-graft.md](references/ray-graft.md).
...
| `severity`   | string | Yes      | `CRITICAL`, `HIGH`, `MEDIUM`, `LOW` (Ray scale) |
...
- SARIF: extract `ruleId`, map `level` to severity (`error`->`HIGH`,
  ...
  `message.text`.
- Semgrep JSON: extract `check_id` as `rule_id`, map `extra.severity` to Ray
...
- **Invocation:** The harness invokes `/ray-graft` with
...
4. **ray-condenser invocation:** Proceeds as usual — the seeded findings are
...
1. Read `active_snapshot` from `workspace/.ray_state.json` (via Block A). If
...
(`/ray-detonator`) has a hard ceiling of 6 attempts per finding (absolute,
...
- **ray-conductor:** Invoke `/ray-graft` with
  `--snapshot_root`/`--snapshot_id`/`--state_root` after `/ray-prospector`
  completes and before `/ray-condenser`.
- **ray-condenser:** No changes needed. Seeded findings are in
...
- **ray-arbiter:** No changes needed. The 13-rule negative filter applies to
  seeded findings identically.
- **ray-chronicle:** No changes needed. The `sast_provenance` field is
...
- **No existing skills modified.** The adapter writes findings to
  `workspace/findings/` before `/ray-condenser` runs.
...
[references/ray-lattice.md](references/ray-lattice.md).

Two implementations are supported, sharing the same query contract:

- **Option A (Default — Skill-Based):** The `ray-lattice` skill
  generates and runs helper scripts (`build_structural_index.py` and
  `query_structural_index.py`, both `# RAY_HELPER_VERSION = 5`) using
...
- **ray-blueprint:** Optionally read the pre-built structural index (built
  by `ray-lattice` at Stage 0.5) during KB synthesis,
  cross-referencing it with `dependencies.json`.
- **ray-compass:** Use the structural index for function-level dependency
  fan-out (more precise than file-level `dependencies.json`).
- **ray-prospector:** Wave 1 sub-agents use `find_callers()` to SUPPLEMENT
...
2. **Inter-Conversation Retries (Fresh Context + Accumulated Artifacts):**
   ...
   - **Context Provisioning:** The orchestrator populates the new prompt with
     ...
     insights from `workspace/insights.jsonl` (e.g., *"Attempt 1 failed due to
...
- **Sandbox Boundary:** `ray-detonator` executes strictly within isolated
...
- **Critic Integration:** The harness records `ingress_blocked: true` (or edge
  ...
  empirical evidence for `/ray-magistrate` to classify `production_viability` as
