# Reference Blueprint: ray-beacon Skill
...
`ray-beacon` skill in their own environment, invoked as a sub-agent by
`ray-compass` or `ray-prospector` when semantic retrieval is available.
...
- **Reads**:
  - `workspace/kb/chunks.jsonl` (the chunk index, produced by the harness or
    `ray-blueprint` post-Stage-2; one JSON object per line, with an
...
4. **Reusable Deterministic Scripting (versioned):** Write a reusable helper
   ...
   exactly `# RAY_HELPER_VERSION = 1`. Before reusing an existing helper,
   grep its first lines for `RAY_HELPER_VERSION = 1`; if that marker is
