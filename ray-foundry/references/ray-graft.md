# Reference Blueprint: ray-graft Skill
...
`ray-graft` skill in their own environment, invoked by the harness
...
verifies provenance against the pinned `CODE_ROOT`, and writes Ray finding
...
- **Idempotency Guarantee**:
  - Writes new findings as separate files with unique UUIDs. Relies on
    `ray-condenser` to cluster and merge duplicates with LLM-generated findings.
...
2. Read `active_snapshot.snapshot_id` from `workspace/.ray_state.json` (if
   present).
...
- `severity`: From IR (already mapped to Ray enum: `CRITICAL`/`HIGH`/
  `MEDIUM`/`LOW`).
...
- `signature`: Computed using the EXACT same formula as `ray-prospector`:
...
2. **Invocation:** The harness calls `/ray-graft` with
...
4. **Downstream:** `ray-condenser` (Stage 7) merges SAST-seeded findings with
