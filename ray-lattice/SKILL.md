...
code-reading analysis stage (Prism/Blueprint). It only needs `CODE_ROOT`
...
- **Reads**:
  - `workspace/.ray_state.json` (to read `active_snapshot` for provenance
...
- **Inert until wired:** This skill returns an empty index until a caller
  invokes it (the harness, `ray-compass`, or `ray-prospector`). It never
...
(`build_structural_index.py`, `# RAY_HELPER_VERSION = 5`, grep-and-regenerate
...
1. Write the builder to `workspace/helpers/build_structural_index.py`. The FIRST
   LINE MUST be exactly `# RAY_HELPER_VERSION = 5`. Before reusing an
   existing helper, grep its first lines for `RAY_HELPER_VERSION = 5`; if
...
1. Write the query helper to `workspace/helpers/query_structural_index.py`. The
   FIRST LINE MUST be exactly `# RAY_HELPER_VERSION = 5`. Before reusing an
   existing helper, grep its first lines for `RAY_HELPER_VERSION = 5`; if
...
workspace/helpers/
├── build_structural_index.py    # Builder (RAY_HELPER_VERSION = 5)
└── query_structural_index.py    # Query helper (RAY_HELPER_VERSION = 5)
...
- Identical operations for local and remote providers (remote via
  `RAY_LATTICE_URL` env var or manifest `provider.kind = "remote"`).
...
[ray-foundry/references/ray-lattice.md](../ray-foundry/references/ray-lattice.md).
