...
name: ray-manifest
description: >-
  Hunts dependency risk: known-vulnerable versions (CVEs), a software bill of materials (CycloneDX), typosquatted or unpinned packages, and malicious install scripts.
  Use when the target has a dependency manifest or lockfile (package.json, requirements.txt, go.mod, Cargo.toml, pom.xml, Gemfile…).
  Don't use for first-party code flaws (the other domain skills).
---
...
Dependency Auditor. Inventories the third-party code the target ships and cross-references it against known-vulnerable versions and advisories — the class the first-party audit cannot see, because the bug is in code you did not write but do run.

## Command Definition

- **Command:** `/ray-manifest [--target_root=<path>] [--snapshot_root=<path>] [--snapshot_id=<id>] [--state_root=<path>]`
- **Description:** Cross-references dependencies against known-vulnerable versions and builds an SBOM.
...
- **Reads**:
  ...
  - `references/manifest-docket.md` — this skill's vulnerable→safe pattern catalogue
...
Read `references/manifest-docket.md`. From `workspace/plan.json`, take the
...
### Step 2: Hunt — the code you ship but did not write

Parse the dependency manifests and lockfiles (direct and transitive). Build an
inventory: package, resolved version, latest version. Cross-reference each against known
advisories/CVEs (OSV, GitHub advisories, ecosystem advisories such as the Vercel/Next
notices) using whatever tooling and network access is available — `osv-scanner`,
`npm audit`, `pip-audit`, `govulncheck`, or the advisory web. Flag: known-vulnerable
resolved versions with a reachable path when determinable; unpinned/floating ranges;
typosquats and lookalike names; suspicious `postinstall`/build scripts. Emit a CycloneDX
SBOM as an artifact when asked.
...
the spec or by execution; (6) it is not a designed behavior for a trusted actor.
 A known-vulnerable dependency version is worth reporting even before app-reachability is proven — but state the reachability honestly; ray-gauge's third_party_reachability cap will down-weight an unproven path, and a target profile may lift it.
