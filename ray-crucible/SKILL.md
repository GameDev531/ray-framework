...
name: ray-crucible
description: >-
  Hunts injection: untrusted input reaching a dangerous sink — SQL/NoSQL, XSS, command, SSTI, XXE, deserialization, SSRF, path traversal, prototype pollution, request smuggling, type juggling, HPP.
  Use when the target parses, stores, renders, or forwards untrusted input.
  Don't use for auth/session logic (ray-turnstile) or native memory bugs (ray-marrow).
---
...
Injection Hunter. Traces every untrusted input from its entry point to a dangerous sink and proves where a missing or wrong transformation lets the attacker change the meaning of a query, document, command, template, path, or request.

## Command Definition

- **Command:** `/ray-crucible [--target_root=<path>] [--snapshot_root=<path>] [--snapshot_id=<id>] [--state_root=<path>]`
- **Description:** Traces untrusted input from entry point to a dangerous sink and reports exploitable injection.
...
- **Reads**:
  ...
  - `references/crucible-docket.md` — this skill's vulnerable→safe pattern catalogue
...
Read `references/crucible-docket.md`. From `workspace/plan.json`, take the
...
### Step 2: Hunt — untrusted input → dangerous sink

Enumerate entry points (HTTP params/body/headers, message queues, files,
env-influenced data, DB rows written by another path) and dangerous sinks (SQL/NoSQL
queries, HTML/JS output, shell/`exec`/`spawn`, template rendering, XML parsing,
deserializers, filesystem paths, outbound URL fetch, object merge/assign). For each
reachable entry→sink pair, decide whether the value is neutralized *for that sink's
grammar* (parameterization, context-correct escaping, allow-list validation, a safe
parser) at every step. Chase indirect flows: data stored safely then used unsafely by
different code; injection through field names, keys, and headers — not just values;
second-order injection into logs, caches, and search indexes.
