...
name: ray-citadel
description: >-
  Hunts deployed-architecture weaknesses: flat networks, missing service isolation, weak service-to-service trust, secrets management at scale, and lateral-movement paths across a multi-service system.
  Use when the target is a multi-service/deployed system, not a single app.
  Don't use for single-file cloud/IaC misconfig (ray-terrain) or app auth (ray-turnstile).
---
...
Architecture Auditor. Steps back from a single service to the deployed topology — the network layering, the trust between services, the blast radius of one compromised component, and how secrets flow across the estate.

## Command Definition

- **Command:** `/ray-citadel [--target_root=<path>] [--snapshot_root=<path>] [--snapshot_id=<id>] [--state_root=<path>]`
- **Description:** Reviews deployed-architecture isolation, service-to-service trust, and secrets at scale.
...
- **Reads**:
  ...
  - `references/citadel-docket.md` — this skill's vulnerable→safe pattern catalogue
...
Read `references/citadel-docket.md`. From `workspace/plan.json`, take the
...
### Step 2: Hunt — blast radius and service-to-service trust

From the architecture KB and deployment descriptors, map services, their network
reachability, and what authenticates a call between them. Ask: if one service is
popped, what else can it reach? Is east-west traffic authenticated and authorized, or
does the perimeter do all the work (flat internal network)? Where do secrets live and
how are they distributed — one shared credential across services, or scoped per
service? Trace the shortest path from an externally-reachable service to the crown
jewels.
